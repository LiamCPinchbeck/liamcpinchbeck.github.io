...
Here, $$U(x) = -\log \pi(x)$$ is still our potential energy (negative log-target). 

The constant $$\sqrt{2}$$ isn't arbitrary; it is the specific scaling required to ensure the stationary distribution of this process is exactly our target $$\pi(x)$$.
...
Let's test if our target $$\pi(x) \propto e^{-U(x)}$$ satisfies this:
...
This MH step "prunes" the errors introduced by the Euler-Maruyama discretization, ensuring that the chain converges to the exact target $$\pi(x)$$ regardless of the step size (though a large $$\epsilon$$ will still lead to many rejections).
...
Invariant here meaning that once the system reaches that distribution, it stays there. i.e. a steady state. If we have a collection of particles distributed according to our target density $$p(z)$$, and apply the dynamics described by your SDE (the drift and the diffusion), the overall shape of that distribution doesn't change over time. And for the ergodicity of these process some sort of jump process has to be added, like the momentum resampling in HMC.

[Ma et al.](https://arxiv.org/abs/1506.04696) then provide an [ansatz](https://en.wikipedia.org/wiki/Ansatz) for required form of the drift term, such that the process converges on our target density $$p(z)$$ or leaves it invariant:
...
For their claims to be true they need to show two things: 1. That the stationary distribution/stead-state solution to the SDE $$p_s(z)$$ is either equivalent to the target distribution $$p(z)$$ or the evolution of the SDE leaves the target distribution invariant. This is shown in theorem 1 in the paper.
