...
With $$C^{new}$$ being samples from the last round of doubling, and $$C^{old}$$ being all the other samples (i.e. $$C/C^{new}$$) what this kernel is quantifying is the probability of moving from a coordinate $$z=(\theta, r) \in C^{old}$$ to $$z'=(\theta', r') \in C^{new}$$.
