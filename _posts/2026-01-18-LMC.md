...
The trajectories share elements of [Gibbs sampling](https://en.wikipedia.org/wiki/Gibbs_sampling). Which if you're unfamiliar, is (kinda secretly) an MCMC method where if you have a distribution such as $$p(\theta_1, \theta_2, ..., \theta_d)$$ then you can sample it by iteratively sampling the conditionals $$p(\theta_i\vert\theta_1, \theta_2, ..., \theta_{i-1}, \theta_{i+1}, ..., \theta_d) = p(\theta_i \vert \theta_{-i})$$ where we define $$\theta_{-i}$$ as all $$\theta_j$$ where $$i\neq j$$. 
