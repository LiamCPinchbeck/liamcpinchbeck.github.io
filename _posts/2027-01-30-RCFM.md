...
- Mathieu & Nickel (2020) - "Continuous Hierarchical Representations with Poincaré Variational Auto-Encoders"
...
- De Bortoli et al. (2022) - "Diffusion Schrödinger Bridge with Applications to Score-Based Generative Modeling"
...
- 1.2 Example: Sky localization with RA/Dec → the pole singularity
- 1.3 Example: Rotations with Euler angles → gimbal lock
- 1.4 Example: Molecular torsion angles → periodic wraparound
...
- 2.4 Molecular conformations (tori Tⁿ for dihedral angles)
- 2.5 3D rotations in robotics and computer vision (SO(3))
- 2.6 Poses and rigid transformations (SE(3))
- 2.7 Hierarchical embeddings (hyperbolic space Hⁿ)
...
- 4.1 Intuition: surfaces that locally look like ℝⁿ
...
- 6.5 Examples: canonical metrics on S², Hⁿ, SO(3)
...
- 8.1 The exponential map: exp_x : T_xM → M
- 8.2 The logarithmic map: log_x : M → T_xM
...
- 9.1 The covariant derivative ∇
- 9.2 Christoffel symbols Γⁱⱼₖ
...
- 11.4 Example: volume of Sⁿ and Hⁿ
...
- 21.1 Wrapping ℝⁿ flows onto Tⁿ
- 21.2 Projecting ℝⁿ⁺¹ flows onto Sⁿ
...
- 25.3 The CFM loss: L(θ) = 𝔼_{t,x₁,x_t} ||v_θ(t, x_t) - u_t(x_t|x₁)||²_g
...
- 26.1 Sampling from p₁ (data distribution)
- 26.2 Sampling from p₀ (base distribution)
- 26.3 Computing the conditional geodesic interpolant
- 26.4 Computing the target vector field u_t
- 26.5 Training the neural vector field v_θ
...
- 28.1 Parameterization: angles θ ∈ [0, 2π)
- 28.2 Metric and geodesics
- 28.3 Exponential map: exp_θ(v) = θ + v (mod 2π)
- 28.4 Logarithmic map: log_θ(φ) = shortest signed distance
- 28.5 Base distributions: wrapped normal, von Mises
- 28.6 Flow matching on S¹: complete worked example
- 28.7 Implementation tricks: handling wraparound
- 28.8 Application: time-of-day, torsion angles

#### 29. The Sphere S²

- 29.1 Embedding in ℝ³: x² + y² + z² = 1
...
#### 30. Higher-Dimensional Spheres Sⁿ
...
#### 32. Hyperbolic Space Hⁿ

- 32.1 Motivation: hierarchies and trees
- 32.2 Models: Poincaré ball, Poincaré half-plane, hyperboloid
- 32.3 The hyperbolic metric (negative curvature)
- 32.4 Geodesics in the Poincaré ball
...
- 34.2 Product manifold structure: SE(3) ≅ SO(3) × ℝ³
...
- 38.1 Product manifolds: M₁ × M₂
- 38.2 Factorized flows vs. coupled flows
- 38.3 Metric structure on products
- 38.4 Example: (ℝ⁺ × ℝ⁺) × S² for GW masses + sky position
...
- 42.3 Fréchet Inception Distance on manifolds
...
- 45.3 Schrödinger bridges on manifolds
...
- 52.1 Problem setup: time series → posterior on S²
...
- 53.1 Poincaré ball model setup
...
- 53.6 Visualization: Poincaré disk plots
...
#### 55. Tutorial: Mixed Geometry Flow (ℝ² × S²)
...
- 66.4 Example: flows on S¹ and winding number
...
- C.1 Core operations on S², Sⁿ, Hⁿ, SO(3), SE(3), Tⁿ
