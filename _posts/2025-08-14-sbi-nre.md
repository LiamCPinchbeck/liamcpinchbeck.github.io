...
for epoch in pbar:
    ...
    for X_batch, y_batch in loader:
        optimizer.zero_grad()
        data, theta = X_batch[:, 0].unsqueeze(1), X_batch[:, 1:]

        y_pred = model(data, theta)
...
with torch.no_grad():
    ...
    prob = torch.sigmoid(model(centres, sample)).mean().item()
...
with torch.no_grad():
    ...
    loglike = model(example_centres, samples).squeeze()
