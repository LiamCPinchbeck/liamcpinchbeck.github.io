...
and we could use those eigen vectors, e.g. $$v_1$$, and project our data on to them using a dot product $$v_1^T \vec{x_i}$$.
...
$$\begin{align}
k(x_i, x_j) = \exp(-\gamma \sum_k \frac{\left(x_i^k - x_j^k)^2}{x_i^k + x_j^k}),
\end{align}$$
