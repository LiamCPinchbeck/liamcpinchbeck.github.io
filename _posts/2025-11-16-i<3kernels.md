...
$$\begin{align}
L(\vec{\alpha}) &= (Y - K \vec{\beta})^2 + \lambda \vec{\beta}^T K \vec{\beta} \\
\end{align}$$

So we can efficiently calculate our loss and project our data onto $$\vec{\alpha}$$ implicitly (unless $$N$$ is large, in which case inverting $$K + \lambda I$$ ain't gonna be easy).
...
The [White Kernel](https://scikit-learn.org/stable/modules/generated/sklearn.gaussian_process.kernels.WhiteKernel.html) has the following form.
...
So the KPCA did worse. This may be because we were more likely to fit to noise/random artefacts (although I did try and regularise and deregularise the KPCA projection), or inability to find a good set of parameters for the classifier or a vast number of things due to my insufficient amount of time to really do everything robustly. In my opinion it's likely the fact that the classifier doesn't match up as well with KPCA as it does with PCA, so it might be interesting to see if using different classifier architectures retain the same behaviour. I might tackle this in a later post but for now I'll leave it here.
