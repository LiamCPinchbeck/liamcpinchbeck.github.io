...
Where $$\vec{\epsilon}\sim \mathcal{N}(0, \sigma_i^2)$$. Splitting it up this way can also be referred to as the [reparameterisation trick](https://en.wikipedia.org/wiki/Reparameterization_trick) where all the stochasticity has movied to $$\vec{\epsilon}$$ now and $$X \vec{\alpha}$$ is deterministic. But that's just a fun little aside.
